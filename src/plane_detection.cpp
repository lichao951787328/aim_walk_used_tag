...
// /camera/depth/image_rect_raw   消息类型sensor_msgs/Image
// bool /* PlaneDetection:: */readDepthImageFromPCLPOINTXYZ(pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud)
// {
// 	// const sensor_msgs::Image::ConstPtr& msg;
// 	const sensor_msgs::Image::ConstPtr msg;
	
// 	int rows = msg->height, cols = msg->width;
...
bool PlaneDetection::runPlaneDetection()
{
	...
	// uint32_t sum = 0.0;
    // for(size_t i = 0; i < plane_vertices_.size(); i++)
    // {
    //     sum += plane_vertices_.at(i).size();
    // }
    // cout<<"in plane detection obj, funtion runPlaneDetection : sum point cloud is "<<sum<<endl;
	...
	// std::cout<<"still in plane detection."<<std::endl;
	// std::cout<<"in plane detection, plane number is "<<plane_num_<<std::endl;
...
{
	cout<<"plane_num_= "<<plane_num_<<endl;
