...
#include <filesystem>
#define DEBUG
string debug_dir = "/home/humanoid/catkin_bhr_ws/src/mutithread_for_walk_outdoor/debugdata/";
// string debug_dir = "/home/humanoid/catkin_bhr_ws/src/mutiThread_perception/debugdata/";
extern ros::Time camera_time;
extern cv::Mat depth_img;
extern cv::Mat color_image;
extern std::queue<sensor_msgs::Imu> imuDatas;
...
bool ubuntu_perception::plane_detection(std::ofstream& fs)
{
  ...
  try
	{
		if(!color_img_needed.empty())
		{
      LOG(INFO)<<color_img_needed.rows<<" "<<color_img_needed.cols<<endl;
      cv::Mat gray_image, Gaussian_image;
      cv::cvtColor(color_img_needed, gray_image, cv::COLOR_RGB2GRAY);
      ...
      // cv::imwrite(debug_dir + std::to_string(index_image) + "pz.png", pz_image);
      LOG(INFO)<<"change to threld image"<<endl;
		}
    cv::waitKey(1);
	}
	catch (cv_bridge::Exception& e)
	{
    LOG(ERROR)<<"can not get threld image"<<endl;
	}
  ...
  assert(cols_end <= cols);
	#pragma omp parallel for reduction(+:plane_detection_bl, vertex_idx, threld_filter_cloud, need_could)
  for (int i = rows_start; i < rows_end; ++i)
	{
		for (int j = cols_start; j < cols_end; ++j)
		{
      // LOG(INFO)<<"the filter"<<endl;
      assert(vertex_idx < rows * cols);
      ...
      if (walk_case == 'A')// 直线行走
      {
        ...
        need_could->emplace_back(pclpoint);
  ...
  LOG(INFO)<<"filted size is "<<need_could->size()<<endl;
  assert(threld_filter_cloud->size() > 0);
  assert(need_could->size() > 0);
  // string save_raw_pcd_path_point = debug_dir + std::to_string(seq_needed) + "_" +"raw_cloud.pcd";
  // pcl::io::savePCDFileASCII(save_raw_pcd_path_point, *raw_cloud);

  string save_threld_pcd_path_point = debug_dir + std::to_string(seq_needed) + "_" + "threld_filter_cloud.pcd";
  pcl::io::savePCDFileASCII(save_threld_pcd_path_point, *threld_filter_cloud);

  string save_need_pcd_path_point = debug_dir + std::to_string(seq_needed) + "_" +"need.pcd";
  pcl::io::savePCDFileASCII(save_need_pcd_path_point, *need_could);
  // finish = clock();
...
// 平面内的点太少，舍弃
  ...
  else if(walk_case == 'C')// 新的对准台阶，没有考虑必须对准台阶中间
  {
    // 20220415对准靠近代码
    ...
    assert(steps_result.size() > 0);
    assert(steps_result.size() % 2 == 0);
    return steps_result.size() > 0;
  }
}
